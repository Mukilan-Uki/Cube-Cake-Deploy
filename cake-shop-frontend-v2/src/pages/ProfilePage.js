...
const ProfilePage = () => {
  const { user, logout, token, setAuthData } = useAuth();
  ...
  // Use profileForm value; fall back to user.profilePicture only if form value is null/undefined (not empty string)
  const profilePic = profileForm.profilePicture != null ? profileForm.profilePicture : (user.profilePicture || '');
              ...
              <>
                ...
                <div className="mb-4 p-4 rounded-4" style={{ background: 'rgba(255,158,109,0.06)', border: '1.5px dashed rgba(255,158,109,0.3)' }}>
                  ...
                  <div className="d-flex align-items-center gap-4">
                    {profilePic ? (
                      /* Picture already set — show preview + remove only */
                      <div className="d-flex align-items-center gap-3">
                        <div style={{
                          width: '70px', height: '70px', borderRadius: '50%', overflow: 'hidden', flexShrink: 0,
                          background: 'linear-gradient(135deg, #FF9E6D, #FF6B8B)',
                          display: 'flex', alignItems: 'center', justifyContent: 'center',
                        ...
                        <div>
                          <p className="mb-1 fw-medium text-chocolate">Profile photo set ✓</p>
                      ...
                      /* No picture — show upload controls */
                      <div className="d-flex align-items-center gap-4">
                        <div style={{
                          width: '70px', height: '70px', borderRadius: '50%', overflow: 'hidden', flexShrink: 0,
                          background: 'linear-gradient(135deg, #FF9E6D, #000000ff)',
                          display: 'flex', alignItems: 'center', justifyContent: 'center',
                          fontSize: '1.6rem', fontWeight: 700, color: 'white'
                        }}>
                          <span>{avatarLetter}</span>
                ...
                <form onSubmit={handleProfileUpdate}>
                  <div className="row g-3">
                    ...
                    <div className="col-md-6">
                      ...
                      <input type="text" className="form-control" value={user.role === 'super_admin' ? 'Super Administrator' : user.role === 'shop_owner' ? 'Shop Owner' : 'Customer'} disabled />
                    </div>
                  </div>
