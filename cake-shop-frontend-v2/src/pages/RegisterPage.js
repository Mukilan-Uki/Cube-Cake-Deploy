...
const RegisterPage = () => {
  ...
  const validate = () => {
    ...
    else if (errors.email === 'This email is already taken') e.email = 'This email is already taken';
        ...
        <form onSubmit={handleSubmit}>
            ...
            <div style={{ position: 'relative' }}>
              ...
              {emailChecking && (
                <div style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)' }}>
                  <span className="spinner-border spinner-border-sm" style={{ width: '0.8rem', height: '0.8rem', color: '#D4AF37' }}></span>
                </div>
              )}
              {!emailChecking && formData.email && !errors.email && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.email) && (
                <i className="bi bi-check-circle-fill" style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', color: '#28a745', fontSize: '0.9rem' }} />
              )}
            </div>
          ...
          <div style={{ marginBottom: '1rem' }}>
            ...
            <div style={{ position: 'relative' }}>
              ...
              <input type="tel" pattern='[0-9]{10}' value={formData.phone} onChange={(e) => {
                const numbersOnly = e.target.value.replace(/\D/g, '');
                setFormData({ ...formData, phone: numbersOnly });
              }} placeholder="+94 77 000 0000" style={inputStyle(false)}
